<nav class="fixed top-0 left-0 w-full bg-white border-b">
    <div class="grid grid-cols-3 items-center max-w-[1300px] mx-auto">
        ...
        @if (auth()->user())
            <div x-data="{open: false}" class="relative justify-self-end">
                <button @click="open = true" class="flex items-baseline border-none rounded-full">
                    ...
                    <svg :class="open ? 'rotate-180' : ''" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 ml-3 transition ease-in-out">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                </button>

                <div x-cloak x-show="open" @click.outside="open = false" class="absolute w-48 p-4 mt-2 bg-white border">
